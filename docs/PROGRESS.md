...
- [ ] **Deploy belum ada.** `landing-page-nuxt` memakai PM2 karena ia proses
      Node; menyalin polanya ke PHP salah. Butuh nginx + php-fpm (atau Octane)
      dan config-nya sendiri di `deploy/`. Yang SUDAH ada:
      [docs/PRODUCTION.md](PRODUCTION.md) — daftar hal yang gagal diam-diam di
      server, ditulis lebih dulu karena ia yang dibutuhkan saat deploy pertama,
      bukan setelahnya.
