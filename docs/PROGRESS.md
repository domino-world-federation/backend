...
| P99 | **Pita "Featured Event" di beranda dipilih FLAG, tanpa saringan tanggal** | Sebelumnya murni derivasi: enam turnamen terdekat yang belum berakhir. Kolom `tournaments.is_featured` + sakelar "Set Featured" di formulir menggantikannya. Lantai `ends_on >= hari ini` sempat dipertahankan di atas flag, lalu **dicabut atas keputusan pemilik repo**: turnamen yang sudah berakhir boleh diunggulkan, karena pita itu menjawab "apa yang dipilih federasi", bukan "apa yang akan datang". Harganya diterima sadar — yang menahan turnamen lama tetap terpampang cuma orang yang ingat mencabut centangnya, dan tidak ada layar yang mengingatkan. Ikutannya: urutan jadi DUA TINGKAT (belum berakhir dulu, paling dekat duluan; lalu yang sudah, paling baru usai duluan), karena `orderBy('starts_on')` polos akan menaruh acara tahun lalu di kartu pertama. Migrasinya menandai hanya turnamen yang belum berakhir, jadi beranda tidak berubah pada hari migrasi. `/tournaments/featured` (countdown S3) dan `/tournaments/highlighted` (hero halaman turnamen) TIDAK ikut — keduanya tetap derivasi tanggal murni |
| P100 | **"Tournament Rules Format" dipecah jadi Tournament Mode + Domino Rules** | Permintaan pemilik repo. Enam nama lama (`Single 101` … `Double BO3`) ternyata hasil silang dua pertanyaan yang berdiri sendiri, dan menyatukannya memaksa orang merakit dua keputusan di kepalanya sebelum memilih — lalu menguraikannya lagi setiap kali membacanya. `Knockout` dan `BO3` ikut berganti nama jadi `1 Round` dan `Double Win`, nama yang dipakai federasi sendiri; mekanismenya tidak berubah. Migrasinya mengonversi keenam nama tanpa tebakan dan `down()` merangkainya kembali. `rules_format` DIBUANG, bukan ditinggal: kolom tanpa pemilik akan tetap diisi kode lama yang terlewat. `formatLabel` di API publik sekarang `"Single · 101"` — titik tengah, karena dirangkai spasi `"Single Double Win"` terbaca sebagai satu nama aturan yang tidak ada |
...
- [x] **Formulir turnamen kembali ke aturan wajib semula** (2026-09-18).
      Percobaan "seluruh field wajib" di hari yang sama (`f8d9f54`) dibatalkan
      atas permintaan pemilik repo; Prize, Contact, jendela pendaftaran, syarat
      DWF ID, foto ofisial, area jadwal, jumlah peserta, dan dokumen kembali
      opsional seperti desain `585:11241`.
